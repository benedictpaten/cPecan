...
def parse_args():
    ...
    parser.add_argument('--C_alignments', '-C', action='store',
                        dest='C_alns', required=True, type=str, default=None,
                        help="C files")
    parser.add_argument('--mC_alignments', '-mC', action='store',
                        dest='mC_alns', required=True, type=str, default=None,
                        help="mC files")
...
def make_build_alignment(c_alns, mc_alns, hmc_alns, strand, threshold, max_assignments):
    ...
    print("NOTICE: I found {C} C-assignments, {mC} mC-assignments, and {hmC} hmC-assignments for strand"
          " {strand}".format(C=C_table.shape[0], mC=mC_table.shape[0], hmC=hmC_table.shape[0], strand=strand),
          file=sys.stderr)
    return pd.concat([C_table, mC_table, hmC_table])


def main(arguments):
    args = parse_args()
    C_alns, mC_alns, hmC_alns = randomly_select_alignments(args.C_alns), \
                                randomly_select_alignments(args.mC_alns), \
                                randomly_select_alignments(args.hmC_alns)
