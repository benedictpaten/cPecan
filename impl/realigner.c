...
PoaInsert *poaDelete_construct(int64_t length, double weight) {
	PoaInsert *poaDelete = st_calloc(1, sizeof(PoaDelete));
...
PoaNode *poaNode_construct(char base) {
	...
	poaNode->deletes = stList_construct(0, (void(*)(void *)) poaDelete_destruct);
	poaNode->base = base; // A = 0, C = 1, G = 2, T = 3, N = 4
	poaNode->baseWeights = st_calloc(5, sizeof(double)); // A = 0, C = 1, G = 2, T = 3, N = 4
...
Poa *poa_getReferenceGraph(char *reference) {
	...
	stList_append(poa, poaNode_construct(0)); // Add source node
	for(int64_t i=0; i<refLength; i++) {
		stList_append(poa->nodes, poaNode_construct(reference[i]));
	}
	stList_append(poa->nodes, poaNode_construct(0)); // Add sink node
...
void poa_augment(Poa *poa, char *read, stList *matches, stList *inserts, stList *deletes) {
	...
	for(int64_t i=0; i<stList_length(matches); i++) {
		...
		node->baseWeights[base] += stInTuple_get(match, 2); // Add base weight to POA node
	}
	...
	for(int64_t i=0; i<stList_length(matches); i++) {
		...
		assert(stSet_search(matches, match) == NULL);
		stSet_insert(matchesSet, match); // Add to matches
	}
	...
	for(int64_t i=0; i<stList_length(inserts); i++) {
		stIntTuple *insertStart = stIntTuples_get(inserts, i); // Start of putative complete-insert

		int64_t j=i+1;
		for(;j<stList_length(inserts); j++) {
			stIntTuple *insertEnd = stIntTuples_get(inserts, j); // End of putative complete-insert
		...
		for(int64_t k=i; k<j; k++) {
			...
			for(int64_t l=k; l<j; l++) {
				...
				for(int64_t m=k; m<l+1; m++) {
					...
					insertWeight = insertWeight < stIntTuple(insert, 2) ? insertWeight : stIntTuple(insert, 2);
				}
				...
				for(int64_t m=0; m<stList_length(leftNode->inserts); m++) {
					...
					if(stString_eq(poaInsert->label, insertLabel)) {
	...
	stList_sort(deletes, deletesCmpFn);
	...
	for(int64_t i=0; i<stList_length(deletes); i++) {
		stIntTuple *deleteStart = stIntTuples_get(inserts, i); // Start of putative complete-delete

		int64_t j=i+1;
		for(;j<stList_length(deletes); j++) {
			stIntTuple *deleteEnd = stIntTuples_get(deletes, j); // End of putative complete-delete
		...
		for(int64_t k=i; k<j; k++) {
			...
			for(int64_t l=k; l<j; l++) {
				...
				for(int64_t m=k; m<l+1; m++) {
					...
					deleteWeight = deleteWeight < stIntTuple(delete, 2) ? deleteWeight : stIntTuple(delete, 2);
				}
...
Poa *poa_realign(stList *reads, char *reference,
	...
	for(int64_t i=0; i<stList_length(reads); i++) {
		...
		getAlignedPairsWithIndels(sM, reference, reads, p, &matches, &inserts, &deletes);

		// Add weights, edges and nodes to the poa
		partialOrderAlgnment_augment(poa, read, matches, inserts, deletes);
